...
- **Form handling**: Use `useActionState(action, initialState)` — returns `[state, formAction, isPending]`. Pass `formAction` to `<form action={}>`. Never use manual `useState` + `onSubmit` + `setIsPending` for forms.
- **Memoization**: Apply `React.memo` on pure display components (table rows, badges, error messages). Use `useMemo` for expensive derived values. Use `useCallback` for event handlers passed as props. Profile before optimizing — don't memoize everything blindly.
- **Custom hooks**: Extract reusable logic into custom hooks. Effector selectors go in `selectors.ts` object, not inline in components.
- **Component composition**: Prefer composition over prop drilling. Split large components into focused sub-components with single responsibility.

## Code Standards (ENFORCED)

- **SOLID**: Single Responsibility (one purpose per file/function), Open/Closed (extend via composition not modification), Liskov (consistent interfaces), Interface Segregation (small focused props), Dependency Inversion (depend on abstractions — Effector events/stores, not concrete implementations)
- **DRY**: Shared utilities in `#/shared/lib/`, no duplicate logic across widgets, extract common patterns
- **KISS**: Simplest solution that works, no premature abstractions, three similar lines > premature helper
