...
**TECHNICAL IMPLEMENTATION (Tailwind CSS 4)**:

This project uses **Tailwind 4.2.2** — CSS-first configuration, no tailwind.config file.

- All styling via **Tailwind utility classes in `className`** — NEVER inline styles, NEVER CSS modules, NEVER styled-components
- Colors via auto-generated utilities from `@theme inline`: `bg-bg-card`, `text-text-primary` — NEVER `bg-[#241C32]`
- Custom animations: `@keyframes` in globals.css + `animate-[name]` arbitrary values (NO `@utility` directive in v4)
- Responsive: mobile-first with `md:` (768px) and `lg:` (1024px) breakpoint prefixes — every component must work at 375px+
- Dark mode: single dark theme via `:root` variables + `@theme inline` — no `dark:` prefix switching needed (app is always dark)
- Fonts: `font-sans` (Geist Sans) and `font-mono` (Geist Mono) via `@theme inline { --font-sans: ...; --font-mono: ...; }`
- Ensure all animations are performant (60fps, `transform`/`opacity` only) and respect `prefers-reduced-motion`
- Create responsive designs that maintain premium feel from 375px mobile to 1440px desktop
- Use CSS custom properties in `:root` + Tailwind `@theme inline` mappings for all design tokens
