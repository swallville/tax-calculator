...
- Forms use `useActionState` (not manual useState + onSubmit)
- `isPending` from `useActionState` reflected in button disabled/aria-busy
- `React.memo` on pure display components (table rows, badges, error messages)
- `useMemo`/`useCallback` used only where profiling shows re-render issues (not blindly)
- Custom hooks extracted to selectors object (not inline useUnit in components)
- No class components, no legacy lifecycle methods

## Code Standards Review

- **SOLID**: Single responsibility per component/function, dependency inversion via Effector
- **DRY**: No duplicate logic across widgets, shared utilities used from `#/shared/lib/`
- **KISS**: No premature abstractions, simplest solution that works, no over-engineering
