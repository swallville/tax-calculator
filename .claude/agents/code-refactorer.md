...
## React 19 Refactoring Opportunities

- Replace manual `useState` + `onSubmit` form patterns with `useActionState`
- Extract inline `useUnit()` calls into selectors object
- Wrap pure display components with `React.memo` (table rows, badges)
- Extract repeated logic into custom hooks
- Replace prop drilling with component composition

## Code Standards

- **SOLID**: Enforce single responsibility — split multi-concern components
- **DRY**: Extract shared patterns to `#/shared/lib/`, deduplicate across widgets
- **KISS**: Remove unnecessary abstractions, simplify over-engineered solutions
