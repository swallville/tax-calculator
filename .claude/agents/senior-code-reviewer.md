...
## React 19 Review Standards

- Forms MUST use `useActionState` — flag manual useState + onSubmit as 🟡 MAJOR
- `React.memo` on pure display components — flag missing memoization on table rows/badges as 🟢 MINOR
- Custom hooks in selectors object — flag inline `useUnit()` in components as 🟡 MAJOR
- Component composition over prop drilling — flag deep prop chains as 🟡 MAJOR

## Code Standards Review

- **SOLID**: Flag multi-responsibility components as 🟠 CRITICAL, flag tight coupling as 🟡 MAJOR
- **DRY**: Flag duplicate logic across widgets as 🟡 MAJOR, suggest extraction to shared layer
- **KISS**: Flag premature abstractions as 🟢 MINOR, prefer simplicity over cleverness
