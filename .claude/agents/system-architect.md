...
- **Form architecture**: `useActionState` centralizes form state + submission + pending in one hook — prefer over manual state management
- **Component composition**: Split large components into focused sub-components with single responsibility. Prefer composition over inheritance.
- **Custom hooks architecture**: Business logic in Effector selectors object, not inline in components. Hooks should be reusable and testable.
- **Memoization architecture**: `React.memo` on pure display components at leaf level. Effector `.map()` on stores for granular subscriptions at root level. Don't memoize between these layers unnecessarily.

## Code Standards

- **SOLID**: Dependency inversion via Effector (components depend on selectors, not stores directly). Single responsibility per FSD layer.
- **DRY**: Shared layer (`#/shared/`) is the single source for utilities, types, API client. No cross-widget duplication.
- **KISS**: Start with the simplest architecture that works. Extract abstractions only when patterns repeat 3+ times.
