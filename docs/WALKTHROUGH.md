...
| **Custom 60-line logger** (`src/shared/lib/logger/logger.ts`) | Structured logging with field-level redaction. Hard-coded `['salary', '*.salary']` redact list makes accidental PII leakage a test-enforced impossibility. Replaced Pino during Phase 8.6 after the adversarial review flagged it as the one non-load-bearing bundle dependency — preserves Pino's numeric level scheme so downstream aggregators need no config changes. |
...
> The custom logger hard-codes `['salary', '*.salary']` in its redact list. If any code path accidentally tries to log a salary field, the `redact()` helper replaces it with `'[Redacted]'` before the entry reaches `console.*`, so no downstream transport can leak the raw value. I verified this with a dedicated unit test that spies on `console.info`, captures the emitted entries, and asserts both that the redact value is present and that the raw numeric salary does not appear anywhere in `JSON.stringify(entry)` — behavioral, not tautological.
...
**Q: Why did you write your own logger instead of using Pino? Isn't that reinventing the wheel?**
A: I was using Pino through Phase 8.5. The Phase 8.5 Devil's Advocate review caught me rationalizing the bundle miss by claiming "every dependency is load-bearing" — and then pointed out that Pino specifically was not. Pino is a logger, not an architectural primitive. The three call sites in this app only use four method signatures (`logger.info`, `logger.warn`, `logger.error`, `logger.debug`) and one configuration option (the `redact` field list). Everything else Pino ships — transports, child loggers, dynamic level changes, structured serializers — the app does not use and will not use at current scope. In Phase 8.6 I replaced Pino with a 60-line custom file at `front-end/src/shared/lib/logger/logger.ts` that preserves the exact public interface so the three call sites did not need to change, preserves the `['salary', '*.salary']` redact contract, and preserves Pino's numeric level scheme (`debug=20, info=30, warn=40, error=50`) so any log aggregator that parsed the previous Pino NDJSON output continues to parse the new output with no config change. The file has zero runtime dependencies beyond `console` and `process.env.NODE_ENV`. The measured bundle delta after the swap was about 4 kilobytes gzipped — smaller than the 10-to-15 kilobyte estimate the reviewer had given, because Pino's browser build is better tree-shaken than I expected. The swap was still worth shipping because it corrects the architectural-honesty claim: the project now honestly says "every dependency except the logger is load-bearing, and the logger we wrote ourselves." I am happy to open the file in the panel if you want to see the 60 lines — the most interesting pieces are the `redact()` helper that shallow-copies the input and guards against descending into non-plain objects, and the `CONSOLE_SINKS` dispatch-table map that routes each level to the matching `console.*` method. The test coverage is also worth mentioning: the Phase 8.6 rewrite of `logger.test.ts` spies on `console.*` and asserts that the raw salary value is absent from `JSON.stringify(entry)` after redaction — the previous tests had only verified the logger was defined and did not throw, which was structurally tautological. Every current logger assertion is behavioral.
...
A: 218 KB gzipped across 9 chunks on first paint after the Phase 8.6 Pino → custom logger swap. I measured it by wiping the `.next/` cache, running a clean `npm run build`, starting `npm start` on an ephemeral port, fetching the production HTML, and summing the gzipped size of every `<script src=>` tag. That is 68 KB over the 150 KB target in my original plan, and the miss is structural. The baseline for this stack is roughly React 19 at 47 KB gzipped, plus the Next.js App Router runtime at 40 KB, plus Effector and effector-react at 15 KB, plus @farfetched with Zod contract support at 20 KB, plus Zod itself at 15 KB, plus the custom logger at essentially zero, plus about 20 KB of app code. That is a floor of roughly 160–170 KB before any optimization, and the remaining 50 KB is Next.js runtime overhead which I judge reasonable. Dropping Effector, @farfetched, or Zod would cost more in architectural integrity than it would save in bytes — each is the foundation of a specific story I defend during this walkthrough. I tried two optimizations before accepting the miss. The first was dynamic-importing the three widgets that only render after user interaction (`LoadingState`, `ErrorState`, `TaxBreakdown`); the measured delta was zero kilobytes because Next.js App Router prefetches dynamically-imported client component chunks on first paint via the RSC payload — splitting them produces a tenth chunk, not a deferred load. I reverted. The second was a CSP nonce migration via `middleware.ts` that would have let me drop `'unsafe-inline'` from `script-src`; the full Playwright suite passed under the strict policy, but the build table showed every route had switched from `○ (Static)` to `ƒ (Dynamic)` because per-request nonces cannot be statically prerendered, and the bundle inflated by 97 KB gzipped. For a public unauthenticated Canadian tax calculator with one Zod-validated numeric input and zero demonstrated XSS vectors, the strict CSP would have defended against attacks that do not exist in this app. I reverted the migration and preserved the Phase 8.5 hardening directives (`object-src 'none'`, `base-uri 'self'`, `form-action 'self'`). One consequential detail from the Phase 8.5 review: I discovered `next.config.ts` had `compress: false`, which would only have been correct if a reverse proxy were handling compression. The Docker Compose topology has no reverse proxy, so every response had actually been going over the wire as roughly 750 KB raw, not the gzipped figure I had been quoting. I set `compress: true`, rebuilt, and verified. The 218 KB figure is now the real wire size, not a theoretical best-case. One more detail worth knowing about: during the Phase 8.6 Pino swap, my first bundle measurement showed 121 KB — I would have been under the 150 KB target for the first time. I did not ship it. The number was structurally implausible for a logger swap, so I wiped `.next/` and rebuilt; the real number was 218 KB. The 121 KB reading had been measurement noise from a partial server response in my ad-hoc test script. Shipping the wrong number would have been a documentation lie discoverable at the panel.
...
A: The custom logger writes structured entries to `console.*` with numeric level tags matching Pino's scheme (`debug=20, info=30, warn=40, error=50`), so any log aggregator — Datadog, Loki, CloudWatch — that knows how to parse Pino-shaped NDJSON parses this output with no config change. I'd add distributed tracing on the API routes via OpenTelemetry as a follow-up, but the synchronous single-API-call architecture here means traces would mostly be a straight line.
