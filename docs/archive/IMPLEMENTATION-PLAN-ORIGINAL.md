...
- **Logging**: Structured logging via a **custom 60-line logger** at `src/shared/lib/logger/logger.ts` — API calls, errors, retries (never salary). Preserves the `['salary', '*.salary']` redact contract and numeric level scheme (debug=20, info=30, warn=40, error=50) for downstream log-aggregator compatibility.
