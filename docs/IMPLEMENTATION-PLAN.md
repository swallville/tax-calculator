...
- **Logging**: Structured logging via **Pino** (`pino` + `pino-pretty`) — API calls, errors, retries (never salary). Repo: https://github.com/pinojs/pino
...
### Step 8.4 — Commit & PR (STOP — await user approval)

| Agent | Task |
|-------|------|
| **git-commit-helper** | Create Conventional Commits for all changes |

Create PR with: summary, requirements coverage matrix, test plan, a11y statement, security statement

### Step 8.5 — PR Review
Run **`/review-team <PR#>`** — 4 reviewers + Devil's Advocate, address HIGH findings
...
| **8.4 Commit & PR** | PENDING (awaiting user go-ahead) | — | Blocked on explicit user approval per plan. Target: git-commit-helper → grouped Conventional Commits → PR with summary + requirements coverage + test plan + a11y + security statement. Repo-local `git config user.email unlisislukasferreira@hotmail.com` + `user.name "Lukas Ferreira"` to be set before first commit. Branch to stay local until GitHub access is recovered. |
